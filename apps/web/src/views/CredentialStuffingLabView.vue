...
import {
  ...
  type CredentialStuffingStep,
  type CredentialStuffingWorkbench,
...
import {
  credentialStuffingNormalPath,
  credentialStuffingScenarioKey,
  createCredentialStuffingLearningProgress,
  createCredentialStuffingVerificationRecord,
  credentialStuffingReviewChecklist,
  formatCredentialStuffingSignal,
  getCredentialStuffingVariantConfig,
  type CredentialStuffingVariantKey,
} from "../labs/credential-stuffing";
...
const config = computed(() => getCredentialStuffingVariantConfig(props.variant));

const workbench = ref<CredentialStuffingWorkbench | null>(null);
const decisions = ref<string[]>([]);
const result = ref<CredentialStuffingResult | null>(null);
...
const orderedSteps = computed<CredentialStuffingStep[]>(() =>
  activeCase.value
...
const signalText = computed(() =>
  result.value
    ? formatCredentialStuffingSignal(result.value.signal)
    : "尚未运行固定评估",
);
...
function applyNormalFlowPath() {
  decisions.value = [...credentialStuffingNormalPath];
  result.value = null;
  actionMessage.value = "已载入修复后正常登录流程路径";
  errorMessage.value = "";
}
...
async function recordProgress() {
  ...
  try {
    await recordLearningProgress(
      ...
      "credential-stuffing",
      session.token,
      createCredentialStuffingLearningProgress(config.value),
    );
  } catch {
    // 学习进度失败不阻断固定场景观察。
  }
...
async function recordVerification(resultValue: CredentialStuffingResult) {
  if (!session.token) {
    return;
  }
  ...
  try {
    await recordVerificationRecord(
      ...
      "credential-stuffing",
      session.token,
      createCredentialStuffingVerificationRecord(config.value, resultValue),
    );
  } catch {
...
<template>
  <section class="page-section two-column credential-stuffing-page">
    <div class="section-heading">
      <p class="eyebrow">auth / credential stuffing</p>
      <h1>{{ config.title }}</h1>
      <p>{{ config.explanation }}</p>

      <div class="variant-switch">
        <RouterLink to="/labs/auth/credential-stuffing/vuln">风险观察版</RouterLink>
        <RouterLink to="/labs/auth/credential-stuffing/fixed">防御复盘版</RouterLink>
      </div>
      ...
      <RouterLink class="text-link" to="/labs/auth/credential-stuffing"
        >返回实验详情</RouterLink
      >
    </div>

    <div class="credential-stuffing-workbench">
      ...
      <template v-else-if="workbench && activeCase">
        ...
        <section
          class="credential-stuffing-status-panel"
          aria-label="固定场景评估状态"
          aria-live="polite"
        >
          ...
          <ol v-if="result" class="record-list">
            <li v-for="step in result.steps" :key="step.stepKey">
              <strong>{{ formatCredentialStuffingSignal(step.signal) }}</strong>
        ...
        <section class="form-panel" aria-label="凭据填充复盘清单">
          <h2>复盘清单</h2>
          <ul class="record-list">
            <li v-for="item in credentialStuffingReviewChecklist" :key="item.key">
