...
import {
  ...
  type SessionHijackingResult,
  type SessionHijackingStep,
  type SessionHijackingWorkbench,
...
import {
  sessionHijackingNormalPath,
  sessionHijackingScenarioKey,
  createSessionHijackingLearningProgress,
  createSessionHijackingVerificationRecord,
  sessionHijackingReviewChecklist,
  formatSessionHijackingSignal,
  getSessionHijackingVariantConfig,
  type SessionHijackingVariantKey,
} from "../labs/session-hijacking";
...
const config = computed(() => getSessionHijackingVariantConfig(props.variant));

const workbench = ref<SessionHijackingWorkbench | null>(null);
const decisions = ref<string[]>([]);
const result = ref<SessionHijackingResult | null>(null);
...
const orderedSteps = computed<SessionHijackingStep[]>(() =>
  activeCase.value
...
const signalText = computed(() =>
  result.value
    ? formatSessionHijackingSignal(result.value.signal)
    : "尚未运行固定评估",
);
...
function applyNormalFlowPath() {
  decisions.value = [...sessionHijackingNormalPath];
  result.value = null;
  actionMessage.value = "已载入修复后正常会话流程路径";
  errorMessage.value = "";
}
...
async function recordProgress() {
  ...
  try {
    await recordLearningProgress(
      ...
      "session-hijacking",
      session.token,
      createSessionHijackingLearningProgress(config.value),
    );
  } catch {
    // 学习进度失败不阻断固定场景观察。
  }
...
async function recordVerification(resultValue: SessionHijackingResult) {
  if (!session.token) {
    return;
  }
  ...
  try {
    await recordVerificationRecord(
      ...
      "session-hijacking",
      session.token,
      createSessionHijackingVerificationRecord(config.value, resultValue),
    );
  } catch {
...
<template>
  <section class="page-section two-column session-hijacking-page">
    <div class="section-heading">
      <p class="eyebrow">auth / session hijacking</p>
      <h1>{{ config.title }}</h1>
      <p>{{ config.explanation }}</p>

      <div class="variant-switch">
        <RouterLink to="/labs/auth/session-hijacking/vuln">风险观察版</RouterLink>
        <RouterLink to="/labs/auth/session-hijacking/fixed">防御复盘版</RouterLink>
      </div>
      ...
      <RouterLink class="text-link" to="/labs/auth/session-hijacking"
        >返回实验详情</RouterLink
      >
    </div>

    <div class="session-hijacking-workbench">
      ...
      <template v-else-if="workbench && activeCase">
        ...
        <section
          class="session-hijacking-status-panel"
          aria-label="固定场景评估状态"
          aria-live="polite"
        >
          ...
          <ol v-if="result" class="record-list">
            <li v-for="step in result.steps" :key="step.stepKey">
              <strong>{{ formatSessionHijackingSignal(step.signal) }}</strong>
        ...
        <section class="form-panel" aria-label="会话劫持复盘清单">
          <h2>复盘清单</h2>
          <ul class="record-list">
            <li v-for="item in sessionHijackingReviewChecklist" :key="item.key">
