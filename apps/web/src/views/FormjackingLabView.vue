...
import {
  ...
  type FormjackingResult,
  type FormjackingStep,
  type FormjackingWorkbench,
...
import {
  formjackingNormalPath,
  formjackingScenarioKey,
  createFormjackingLearningProgress,
  createFormjackingVerificationRecord,
  formjackingReviewChecklist,
  formatFormjackingSignal,
  getFormjackingVariantConfig,
  type FormjackingVariantKey,
} from "../labs/formjacking";
import { useSessionStore } from "../stores/session";

const props = defineProps<{
  variant: FormjackingVariantKey;
}>();

const session = useSessionStore();
const config = computed(() => getFormjackingVariantConfig(props.variant));

const workbench = ref<FormjackingWorkbench | null>(null);
const decisions = ref<string[]>([]);
const result = ref<FormjackingResult | null>(null);
...
const orderedSteps = computed<FormjackingStep[]>(() =>
  activeCase.value
    ? [...activeCase.value.steps].sort((a, b) => a.order - b.order)
    : [],
);
const signalText = computed(() =>
  result.value ? formatFormjackingSignal(result.value.signal) : "尚未运行固定评估",
);
...
function applyNormalFlowPath() {
  decisions.value = [...formjackingNormalPath];
  result.value = null;
  actionMessage.value = "已载入修复后正常提交流程路径";
  errorMessage.value = "";
}
...
async function recordProgress() {
  ...
  try {
    await recordLearningProgress(
      ...
      "formjacking",
      session.token,
      createFormjackingLearningProgress(config.value),
    );
  } catch {
    // 学习进度失败不阻断固定场景观察。
  }
...
async function recordVerification(resultValue: FormjackingResult) {
  if (!session.token) {
    return;
  }
  ...
  try {
    await recordVerificationRecord(
      ...
      "formjacking",
      session.token,
      createFormjackingVerificationRecord(config.value, resultValue),
    );
  } catch {
...
<template>
  <section class="page-section two-column formjacking-page">
    <div class="section-heading">
      <p class="eyebrow">client / formjacking</p>
      <h1>{{ config.title }}</h1>
      <p>{{ config.explanation }}</p>

      <div class="variant-switch">
        <RouterLink to="/labs/client/formjacking/vuln">风险观察版</RouterLink>
        <RouterLink to="/labs/client/formjacking/fixed">防御复盘版</RouterLink>
      </div>
      ...
      <RouterLink class="text-link" to="/labs/client/formjacking"
        >返回实验详情</RouterLink
      >
    </div>

    <div class="formjacking-workbench">
      ...
      <template v-else-if="workbench && activeCase">
        ...
        <section
          class="formjacking-status-panel"
          aria-label="固定场景评估状态"
          aria-live="polite"
        >
          ...
          <ol v-if="result" class="record-list">
            <li v-for="step in result.steps" :key="step.stepKey">
              <strong>{{ formatFormjackingSignal(step.signal) }}</strong>
        ...
        <section class="form-panel" aria-label="Formjacking 复盘清单">
          <h2>复盘清单</h2>
          <ul class="record-list">
            <li v-for="item in formjackingReviewChecklist" :key="item.key">
