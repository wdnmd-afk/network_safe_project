...
import {
  bflaNormalPath,
  bflaScenarioKey,
  createBflaLearningProgress,
  createBflaVerificationRecord,
  bflaReviewChecklist,
  formatBflaSignal,
  getBflaVariantConfig,
  type BflaVariantKey,
} from "../labs/bfla";
import { useSessionStore } from "../stores/session";

const props = defineProps<{
  variant: BflaVariantKey;
}>();

const session = useSessionStore();
const config = computed(() => getBflaVariantConfig(props.variant));

const workbench = ref<BflaWorkbench | null>(null);
const decisions = ref<string[]>([]);
const result = ref<BflaResult | null>(null);
...
const orderedSteps = computed<BflaStep[]>(() =>
  activeCase.value
    ? [...activeCase.value.steps].sort((a, b) => a.order - b.order)
    : [],
);
const signalText = computed(() =>
  result.value ? formatBflaSignal(result.value.signal) : "尚未运行固定评估",
);
...
function applyNormalFlowPath() {
  decisions.value = [...bflaNormalPath];
  result.value = null;
  actionMessage.value = "已载入修复后正常管理流程路径";
  errorMessage.value = "";
}
...
async function recordProgress() {
  ...
  try {
    await recordLearningProgress(
      ...
      session.token,
      createBflaLearningProgress(config.value),
    );
  } catch {
    // 学习进度失败不阻断固定场景观察。
  }
...
async function recordVerification(resultValue: BflaResult) {
  if (!session.token) {
    return;
  }
  ...
  try {
    await recordVerificationRecord(
      ...
      "functional-authorization",
      session.token,
      createBflaVerificationRecord(config.value, resultValue),
    );
  } catch {
...
<template>
  <section class="page-section two-column bfla-page">
    <div class="section-heading">
      <p class="eyebrow">api / functional authorization</p>
      <h1>{{ config.title }}</h1>
      <p>{{ config.explanation }}</p>

      <div class="variant-switch">
        <RouterLink to="/labs/api/functional-authorization/vuln"
          >风险观察版</RouterLink
        >
        <RouterLink to="/labs/api/functional-authorization/fixed"
          >防御复盘版</RouterLink
        >
      </div>
      ...
      <RouterLink class="text-link" to="/labs/api/functional-authorization"
        >返回实验详情</RouterLink
      >
    </div>

    <div class="bfla-workbench">
      ...
      <template v-else-if="workbench && activeCase">
        ...
        >
          ...
          <ol v-if="result" class="record-list">
            <li v-for="step in result.steps" :key="step.stepKey">
              <strong>{{ formatBflaSignal(step.signal) }}</strong>
        ...
        <section class="form-panel" aria-label="API 功能级授权复盘清单">
          <h2>复盘清单</h2>
          <ul class="record-list">
            <li v-for="item in bflaReviewChecklist" :key="item.key">
