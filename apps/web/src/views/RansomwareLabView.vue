...
import {
  ransomwareNormalPath,
  ransomwareScenarioKey,
  createRansomwareLearningProgress,
  createRansomwareVerificationRecord,
  ransomwareReviewChecklist,
  formatRansomwareSignal,
  getRansomwareVariantConfig,
  type RansomwareVariantKey,
} from "../labs/ransomware";
import { useSessionStore } from "../stores/session";

const props = defineProps<{
  variant: RansomwareVariantKey;
}>();

const session = useSessionStore();
const config = computed(() => getRansomwareVariantConfig(props.variant));

const workbench = ref<RansomwareWorkbench | null>(null);
const decisions = ref<string[]>([]);
const result = ref<RansomwareResult | null>(null);
...
const orderedSteps = computed<RansomwareStep[]>(() =>
  activeCase.value
    ? [...activeCase.value.steps].sort((a, b) => a.order - b.order)
    : [],
);
const signalText = computed(() =>
  result.value ? formatRansomwareSignal(result.value.signal) : "尚未运行固定评估",
);
...
function applyNormalFlowPath() {
  decisions.value = [...ransomwareNormalPath];
  result.value = null;
  actionMessage.value = "已载入修复后正常恢复流程路径";
  errorMessage.value = "";
}
...
async function recordProgress() {
  ...
  try {
    await recordLearningProgress(
      ...
      "ransomware",
      session.token,
      createRansomwareLearningProgress(config.value),
    );
  } catch {
    // 学习进度失败不阻断固定场景观察。
  }
...
async function recordVerification(resultValue: RansomwareResult) {
  if (!session.token) {
    return;
  }
  ...
  try {
    await recordVerificationRecord(
      ...
      "ransomware",
      session.token,
      createRansomwareVerificationRecord(config.value, resultValue),
    );
  } catch {
...
<template>
  <section class="page-section two-column ransomware-page">
    <div class="section-heading">
      <p class="eyebrow">malware / ransomware</p>
      <h1>{{ config.title }}</h1>
      <p>{{ config.explanation }}</p>

      <div class="variant-switch">
        <RouterLink to="/labs/malware/ransomware/vuln">风险观察版</RouterLink>
        <RouterLink to="/labs/malware/ransomware/fixed">防御复盘版</RouterLink>
      </div>
      ...
      <RouterLink class="text-link" to="/labs/malware/ransomware"
        >返回实验详情</RouterLink
      >
    </div>

    <div class="ransomware-workbench">
      ...
      <template v-else-if="workbench && activeCase">
        ...
        >
          ...
          <ol v-if="result" class="record-list">
            <li v-for="step in result.steps" :key="step.stepKey">
              <strong>{{ formatRansomwareSignal(step.signal) }}</strong>
        ...
        <section class="form-panel" aria-label="勒索软件复盘清单">
          <h2>复盘清单</h2>
          <ul class="record-list">
            <li v-for="item in ransomwareReviewChecklist" :key="item.key">
