...
import {
  oauthNormalPath,
  oauthScenarioKey,
  createOauthLearningProgress,
  createOauthVerificationRecord,
  oauthReviewChecklist,
  formatOauthSignal,
  getOauthVariantConfig,
  type OauthVariantKey,
} from "../labs/oauth";
import { useSessionStore } from "../stores/session";

const props = defineProps<{
  variant: OauthVariantKey;
}>();

const session = useSessionStore();
const config = computed(() => getOauthVariantConfig(props.variant));

const workbench = ref<OauthWorkbench | null>(null);
...
const orderedSteps = computed<OauthStep[]>(() =>
  activeCase.value
    ? [...activeCase.value.steps].sort((a, b) => a.order - b.order)
    : [],
);
const signalText = computed(() =>
  result.value ? formatOauthSignal(result.value.signal) : "尚未运行固定评估",
);
...
function applyNormalFlowPath() {
  decisions.value = [...oauthNormalPath];
  result.value = null;
  actionMessage.value = "已载入修复后正常授权流程路径";
  errorMessage.value = "";
}
...
async function recordProgress() {
  ...
  try {
    await recordLearningProgress(
      ...
      "oauth",
      session.token,
      createOauthLearningProgress(config.value),
    );
  } catch {
    // 学习进度失败不阻断固定场景观察。
  }
...
async function recordVerification(resultValue: OauthResult) {
  if (!session.token) {
    return;
  }
  ...
  try {
    await recordVerificationRecord(
      ...
      "oauth",
      session.token,
      createOauthVerificationRecord(config.value, resultValue),
    );
  } catch {
...
<template>
  <section class="page-section two-column oauth-page">
    <div class="section-heading">
      <p class="eyebrow">auth / oauth</p>
      <h1>{{ config.title }}</h1>
      <p>{{ config.explanation }}</p>

      <div class="variant-switch">
        <RouterLink to="/labs/auth/oauth/vuln">风险观察版</RouterLink>
        <RouterLink to="/labs/auth/oauth/fixed">防御复盘版</RouterLink>
      </div>
      ...
      <RouterLink class="text-link" to="/labs/auth/oauth"
        >返回实验详情</RouterLink
      >
    </div>
    ...
    <div class="oauth-workbench">
      ...
      <template v-else-if="workbench && activeCase">
        ...
        >
          ...
          <ol v-if="result" class="record-list">
            <li v-for="step in result.steps" :key="step.stepKey">
              <strong>{{ formatOauthSignal(step.signal) }}</strong>
        ...
        <section class="form-panel" aria-label="OAuth 复盘清单">
          <h2>复盘清单</h2>
          <ul class="record-list">
            <li v-for="item in oauthReviewChecklist" :key="item.key">
