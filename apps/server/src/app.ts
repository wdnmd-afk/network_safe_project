...
import {
  ...
  type ClickjackingEvaluationResult,
  type ClickjackingLabService,
  type ClickjackingVariantKey,
} from "./services/clickjacking-lab.js";
import {
  openRedirectScenarioKey,
  createOpenRedirectLabService,
  type OpenRedirectEvaluationResult,
  type OpenRedirectLabService,
  type OpenRedirectVariantKey,
} from "./services/open-redirect-lab.js";
import {
  credentialStuffingScenarioKey,
  createCredentialStuffingLabService,
  type CredentialStuffingEvaluationResult,
  type CredentialStuffingLabService,
...
import {
  ...
  type OauthEvaluationResult,
  type OauthLabService,
  type OauthVariantKey,
} from "./services/oauth-lab.js";
import {
  formjackingScenarioKey,
  createFormjackingLabService,
  type FormjackingEvaluationResult,
  type FormjackingLabService,
...
export function createApp(options: CreateAppOptions = {}) {
  ...
  const clickjackingLabService =
    options.clickjackingLabService ?? createClickjackingLabService();
  const openRedirectLabService =
    options.openRedirectLabService ?? createOpenRedirectLabService();
  ...
  function readClickjackingVariantKey(
    value: string,
  ): ClickjackingVariantKey | undefined {
    return value === "vuln" || value === "fixed" ? value : undefined;
  }

  function readOpenRedirectVariantKey(
    value: string,
  ): OpenRedirectVariantKey | undefined {
    return value === "vuln" || value === "fixed" ? value : undefined;
  }
  ...
  function readOauthVariantKey(
    value: string,
  ): OauthVariantKey | undefined {
    return value === "vuln" || value === "fixed" ? value : undefined;
  }
  ...
  app.get("/api/labs/web/clickjacking/workbench", (_req, res) => {
    res.status(200).json({
      status: "ok",
      workbench: clickjackingLabService.getWorkbench(),
    });
  });

  app.post(
    "/api/labs/web/clickjacking/:variant/evaluate",
    async (req, res, next) => {
      try {
        const currentUser = await readCurrentUser(req);
        ...
        const variantKey = readClickjackingVariantKey(req.params.variant);

        if (!variantKey) {
          res.status(404).json({
            status: "error",
            message: "clickjacking variant not found",
          });
          return;
        }
        ...
        const result = clickjackingLabService.evaluate({
          variantKey,
          scenarioKey,
          decisions,
        });
  ...
  app.get("/api/labs/web/open-redirect/workbench", (_req, res) => {
    res.status(200).json({
      status: "ok",
      workbench: openRedirectLabService.getWorkbench(),
    });
  });

  app.post(
    "/api/labs/web/open-redirect/:variant/evaluate",
    async (req, res, next) => {
      try {
        const currentUser = await readCurrentUser(req);
        ...
        const variantKey = readOpenRedirectVariantKey(req.params.variant);

        if (!variantKey) {
          res.status(404).json({
            status: "error",
            message: "open redirect variant not found",
          });
          return;
        }
        ...
        const result = openRedirectLabService.evaluate({
          variantKey,
          scenarioKey,
          decisions,
        });
  ...
  app.get("/api/labs/auth/credential-stuffing/workbench", (_req, res) => {
    res.status(200).json({
      status: "ok",
      workbench: credentialStuffingLabService.getWorkbench(),
    });
  });

  app.post(
    "/api/labs/auth/credential-stuffing/:variant/evaluate",
    async (req, res, next) => {
      try {
        const currentUser = await readCurrentUser(req);
        ...
        const variantKey = readCredentialStuffingVariantKey(req.params.variant);

        if (!variantKey) {
          res.status(404).json({
            status: "error",
            message: "credential stuffing variant not found",
          });
          return;
        }
  ...
  app.get("/api/labs/auth/oauth/workbench", (_req, res) => {
    res.status(200).json({
      status: "ok",
      workbench: oauthLabService.getWorkbench(),
    });
  });

  app.post(
    "/api/labs/auth/oauth/:variant/evaluate",
    async (req, res, next) => {
      try {
        const currentUser = await readCurrentUser(req);
        ...
        const variantKey = readOauthVariantKey(req.params.variant);

        if (!variantKey) {
          res.status(404).json({
            status: "error",
            message: "oauth variant not found",
          });
          return;
        }
        ...
        const result = oauthLabService.evaluate({
          variantKey,
          scenarioKey,
          decisions,
        });
  ...
  app.get("/api/labs/client/formjacking/workbench", (_req, res) => {
    res.status(200).json({
      status: "ok",
      workbench: formjackingLabService.getWorkbench(),
    });
  });

  app.post(
    "/api/labs/client/formjacking/:variant/evaluate",
    async (req, res, next) => {
      try {
        const currentUser = await readCurrentUser(req);
        ...
        const variantKey = readFormjackingVariantKey(req.params.variant);

        if (!variantKey) {
          res.status(404).json({
            status: "error",
            message: "formjacking variant not found",
          });
          return;
        }
  ...
  app.get(
    ...
    (_req, res) => {
      res.status(200).json({
        status: "ok",
        workbench: workflowBypassLabService.getWorkbench(),
      });
    },
  );

  app.post(
    "/api/labs/business-logic/workflow-bypass/:variant/evaluate",
    async (req, res, next) => {
      try {
        const currentUser = await readCurrentUser(req);
        ...
        const variantKey = readWorkflowBypassVariantKey(req.params.variant);

        if (!variantKey) {
          res.status(404).json({
            status: "error",
            message: "workflow bypass variant not found",
          });
          return;
        }
