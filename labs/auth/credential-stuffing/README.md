...
本实验只使用固定案例 `reused-credential-batch`，用于观察认证只判断单次口令结果且没有设备、速率和泄露凭据风险关联，并对比漏洞版与修复版的后端判定、学习信号和统一事件日志。
...
1. 访问 `/labs/auth/credential-stuffing/vuln`。
2. 使用固定案例和默认控制策略观察风险被接受。
3. 切换到 `/labs/auth/credential-stuffing/fixed`，观察高风险动作被阻断。
4. 选择“自适应挑战已完成”，确认正常受控流程仍可继续。
