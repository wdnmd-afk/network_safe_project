...
当前切片验证 `social/spear-phishing` 后端固定案例 API、`in-progress` 元数据、文档入口、目录结构和安全边界是否一致。

本轮验证后端 API 和事件日志安全摘要，不验证页面、脚本或只读一致性验证，因为这些能力尚未实现。
...
- 本地元数据总数同步更新。
- 元数据只登记 docs 和 api 入口。
- 脚本目录只包含 README。
- 当前没有前端页面、`exploit.py`、`verify.ts`、真实投递、画像采集、凭据收集、模板生成或第三方平台调用能力。
