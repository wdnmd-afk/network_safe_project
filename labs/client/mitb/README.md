...
通过固定交易视图差异学习浏览器内篡改与交易签名。

本实验只使用固定案例 `synthetic-transaction-view-mismatch`，用于观察高风险交易确认完全依赖可能被篡改的浏览器上下文，并对比漏洞版与修复版的后端判定、学习信号和统一事件日志。
...
1. 访问 `/labs/client/mitb/vuln`。
2. 使用固定案例和默认控制策略观察风险被接受。
3. 切换到 `/labs/client/mitb/fixed`，观察高风险动作被阻断。
4. 选择“交易签名已验证”，确认正常受控流程仍可继续。
