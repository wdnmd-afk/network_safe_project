...
通过固定广告供应链记录学习第三方内容、重定向和隔离策略。

本实验只使用固定案例 `synthetic-ad-chain-change`，用于观察站点将第三方广告内容置于过宽浏览器权限中，并对比漏洞版与修复版的后端判定、学习信号和统一事件日志。
...
1. 访问 `/labs/client/malvertising/vuln`。
2. 使用固定案例和默认控制策略观察风险被接受。
3. 切换到 `/labs/client/malvertising/fixed`，观察高风险动作被阻断。
4. 选择“内容沙箱已验证”，确认正常受控流程仍可继续。
