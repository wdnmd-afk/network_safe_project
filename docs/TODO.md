...
- 总队列：52 项
- 已完成：48 / 52
- 当前状态：`LT-048` 已补齐 README 从零安装与运行章节（第 2.1 节）并补全服务端环境变量示例；契约门禁（`LT-046`／`LT-047`）稳定在 17 组配对 170 项断言；78 个实验全部 `ready`
- 待验证：无（`LT-048` 的命令引用经脚本核对、预期输出经实测确认）
- 下一轮队列：`LT-049` 数据库 schema／迁移一致性检查，随后 `LT-050` 依赖与版本治理（含 pnpm 声明与 lockfile 矛盾）、`LT-051` 平台状态页、`LT-052` 覆盖率基线
- 计数规则：只有实现、文档及约定验证全部完成并回填证据后，任务才计入已完成。

# 2026-08-31 最新进展：LT-048 README 从零安装章节与环境变量示例
