...
| 捕鲸攻击 | 后端受控 API 阶段 | `case-study` / 固定高层决策线索 / 固定防御流程复盘 / 受控 review API / 事件日志安全摘要 | `labs/social/whaling/meta.json`、`labs/social/whaling/docs/fixed-cases.md`、`apps/server/src/services/whaling-lab.ts`、`apps/server/tests/whaling-lab.test.ts`、`docs/execution/2026-07-09-social-whaling-fixed-case-api.md` | `labs/social/whaling/` |
...
- 当前已完成页面差异验证切片：`docs/execution/2026-07-03-social-spear-phishing-playwright-verification.md`、`packages/testing/tests/e2e/platform.spec.mjs`。
- 当前已完成只读一致性验证切片：`docs/execution/2026-07-03-social-spear-phishing-readonly-verification.md`、`tools/lab-scripts/social/spear-phishing/verify.ts`。
- 当前已完成 case-study ready 收口审计：`docs/execution/2026-07-03-social-spear-phishing-ready-closeout.md`。
- 当前元数据为 `ready`，已登记 docs、web、api、Playwright 页面差异验证证据和只读 scripts 入口。
...
- 当前脚本目录只提供 README 边界说明和 `verify.ts` 只读一致性验证，不提供 `exploit.py`、投递脚本、画像采集脚本、模板生成脚本或第三方平台连接脚本。
- 当前 Playwright 只登录本机演示账号、访问固定漏洞版 / 修复版路由、点击固定按钮并断言页面无文本输入框。
...
- 当前已完成目录与 `planned` 元数据切片：`docs/execution/2026-07-09-social-whaling-directory-metadata.md`、`labs/social/whaling/meta.json`。
...
- 当前元数据登记 docs 和 api 入口，`entrypoints.web`、`entrypoints.scripts` 均为空数组。
- 当前不创建页面、`verify.ts`、`exploit.py` 或自动化攻击能力。
...
- 后续若继续实现，应进入前端固定案例工作台切片，只提供固定案例选择器和固定核验策略选择器；继续不创建真实投递、真实画像采集、凭据收集、模板生成、第三方平台调用或攻击脚本能力。
...
- 当前已建立 `labs/infrastructure/misconfiguration/` 目录与 `ready` 元数据，登记 docs、web、api 和只读 scripts 入口，不创建真实配置文件。
- 当前已新增脚本目录边界说明和只读一致性验证入口：`tools/lab-scripts/infrastructure/misconfiguration/README.md`、`tools/lab-scripts/infrastructure/misconfiguration/verify.ts`；不提供 `exploit.py`、扫描器或真实基础设施连接能力。
...
- 当前已接入 Playwright 页面级差异验证：`packages/testing/tests/e2e/platform.spec.mjs`，覆盖漏洞版固定暴露信号和修复版固定审计路径，并断言页面没有任意文本输入框。
- 当前已接入本机只读一致性验证脚本：`tools/lab-scripts/infrastructure/misconfiguration/verify.ts`，只读取仓库内文件，不读取真实配置、不扫描、不连接真实管理接口。
- 当前已完成 simulation ready 收口审计：`docs/execution/2026-07-03-infrastructure-misconfiguration-ready-closeout.md`。ready 只代表本项目内固定配置样例学习闭环完成，仍不创建攻击脚本或真实基础设施连接能力。
...
1. 后续社会工程学扩展案例的边界设计：`social/spear-phishing` 已完成边界设计、`planned` 元数据、固定案例文档、后端固定案例 API、前端固定案例工作台、页面差异验证、只读一致性验证和 case-study ready 收口切片；`social/whaling` 已完成边界设计、目录与 `planned` 元数据、固定案例文档和后端固定案例 API 切片，下一步可进入前端固定案例工作台切片。
2. 后续供应链扩展案例的边界设计。
3. 后续基础设施扩展案例的边界设计。
