...
| DNS 劫持 / 污染 | planned | 内存解析表 / 本机模拟 / 案例化演示 | `labs/network/dns-hijack/meta.json`、`labs/network/dns-hijack/README.md`、`docs/execution/2026-07-01-network-dns-hijack-directory-metadata.md` | `labs/network/dns-hijack/` |
...
- 当前已建立 planned 文档入口：`labs/network/dns-hijack/meta.json`。
...
1. `network/dns-hijack` 后端受控内存解析 API 切片。
