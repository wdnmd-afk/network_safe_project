...
| 捕鲸攻击 | 固定案例文档阶段 | `case-study` / 固定高层决策线索 / 固定防御流程复盘 / docs-only 元数据 / 固定案例卡 | `labs/social/whaling/meta.json`、`labs/social/whaling/docs/fixed-cases.md`、`docs/execution/2026-07-09-social-whaling-fixed-cases.md` | `labs/social/whaling/` |
...
- 当前元数据只登记 docs 入口，`entrypoints.web`、`entrypoints.api`、`entrypoints.scripts` 均为空数组。
- 当前不创建页面、API、数据库写入、事件日志写入、`verify.ts`、`exploit.py` 或自动化攻击能力。
...
- 后续若进入实现，应先做后端固定案例 API 切片，只读取固定案例 key 和固定核验策略 key，并接入统一事件日志安全摘要；继续不创建真实投递、真实画像采集、凭据收集、模板生成、第三方平台调用或攻击脚本能力。
...
1. 后续社会工程学扩展案例的边界设计：`social/spear-phishing` 已完成边界设计、`planned` 元数据、固定案例文档、后端固定案例 API、前端固定案例工作台、页面差异验证、只读一致性验证和 case-study ready 收口切片；`social/whaling` 已完成边界设计、目录与 `planned` 元数据切片和固定案例文档切片，下一步可进入后端固定案例 API 切片。
2. 后续供应链扩展案例的边界设计。
3. 后续基础设施扩展案例的边界设计。
