...
| 实验 | 状态 | 推荐模式 | 当前规划落点 | 后续目录 |
|---|---|---|---|---|
| 配置错误利用 | 页面差异验证阶段 | 本机模拟 / 静态配置分析 / 固定配置审计样例 / 前端固定选择器 / 受控 audit API / Playwright 页面级差异验证 / 事件日志安全摘要 | `apps/web/src/views/MisconfigurationLabView.vue`、`apps/web/src/api/misconfiguration-lab.ts`、`apps/web/src/labs/misconfiguration.ts`、`apps/server/src/services/misconfiguration-lab.ts`、`apps/server/tests/misconfiguration-lab.test.ts`、`packages/testing/tests/e2e/platform.spec.mjs`、`labs/infrastructure/misconfiguration/meta.json`、`docs/execution/2026-07-03-infrastructure-misconfiguration-playwright-verification.md` | `labs/infrastructure/misconfiguration/` |
...
- 下一步切片建议：进入本机只读一致性验证，或执行 simulation ready 收口审计，继续保持不提供攻击脚本和真实基础设施连接能力。
...
1. `infrastructure/misconfiguration` 本机只读一致性验证或 simulation ready 收口审计。
2. 后续社会工程学扩展案例的边界设计。
3. 后续供应链扩展案例的边界设计。
