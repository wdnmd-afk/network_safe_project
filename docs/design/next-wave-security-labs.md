...
|---|---|---|---|---|
| 端口扫描 | 页面级验证已接入 | 固定虚拟资产 / 受控 API / 前端固定选择器 / Playwright 差异验证 | `labs/network/port-scan/meta.json`、`apps/web/src/views/PortScanLabView.vue`、`packages/testing/tests/e2e/platform.spec.mjs`、`apps/server/src/services/port-scan-lab.ts`、`docs/execution/2026-07-01-network-port-scan-playwright-verification.md` | `labs/network/port-scan/` |
...
1. `network/port-scan` 只读一致性验证脚本或 ready 收口审计。
2. `network/dns-hijack` 实现执行文档。
3. `ai/prompt-injection` 实现执行文档。
4. `social/phishing` 案例化执行文档。
6. `supply-chain/dependency-confusion` 模拟实验执行文档。
7. `infrastructure/misconfiguration` 模拟实验执行文档。
