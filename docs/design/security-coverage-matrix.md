...
> 首次建立：2026-07-23
>
> 数据基线：66 个实验（65 个 `ready`、1 个 `in-progress`）、10 个分类、132 个变体
...
既有 65 个 `ready` 主题至少具备 E1–E5；新增的 BFLA 主题已补齐 E1–E5 文件，仍等待命令验证后再推进到 `ready`。标注 `+E6` 的主题还具备页面级验证。
...
| 场景 | 模式 | 深度/实现 | 参考体系 | 证据 | 长期动作 |
|---|---|---|---|---|---|
| `api.functional-authorization` | interactive | D4 专用交互 | OWASP API5:2023 BFLA；CWE-285 | E1–E5（待验证） | LT-021 正在收口两步功能级授权决策、事件摘要和独立验证 |
...
**判断：**勒索软件已形成专用证据分析样板，其余主题仍为引导式案例。后续应继续使用固定进程树、文件事件、时间线和检测规则，不创建真实样本。
...
这些领域不应伪装成当前 66 个场景的完整覆盖，后续应单独编写执行文档。

| 缺口领域 | 当前状态 | 优先级 | 首批建议 |
|---|---|---|---|
| API 安全 | 已新增独立分类；BFLA 首个样板处于 `in-progress` | P0 | 完成 BFLA 验证后继续属性级授权、资源消耗、Webhook 重放 |
...
- 当前 66 个场景全部进入矩阵，其中既有 65 个为 `ready`，BFLA 为 `in-progress`。
- Web/注入和认证授权是成熟度最高的两条主线。
- API 已有首个专用样板；云原生、密码学、业务逻辑、Windows 主机和检测响应仍是主要结构性薄弱点。
