...
> 首次建立：2026-07-23
>
> 数据基线：67 个 `ready` 实验、11 个分类、134 个变体
...
当前 67 个 `ready` 主题至少具备 E1–E5；workflow-bypass 已通过专用只读验证和统一验证入口收口。标注 `+E6` 的主题还具备页面级验证。
...
这些领域不应伪装成当前 67 个场景的完整覆盖，后续应单独编写执行文档。
...
- 当前 67 个场景全部进入矩阵并达到 `ready`，workflow-bypass 已完成 LT-022 收口验证。
- Web/注入和认证授权是成熟度最高的两条主线。
- API 与业务逻辑已有首个专用样板；云原生、密码学、Windows 主机和检测响应仍是主要结构性薄弱点。
