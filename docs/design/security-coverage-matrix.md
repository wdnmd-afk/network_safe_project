...
> 首次建立：2026-07-23
>
> 数据基线：68 个 `ready` 实验、12 个分类、136 个变体
...
当前 68 个 `ready` 主题至少具备 E1–E5；`crypto.insecure-randomness` 已通过晋升前后两轮专项和统一验证完成收口。标注 `+E6` 的主题还具备页面级验证。
...
这些领域不应伪装成当前 68 个场景的完整覆盖，后续应单独编写执行文档。
...
- 当前 68 个场景全部进入矩阵并达到 `ready`，`crypto.insecure-randomness` 已通过 LT-023 晋升前后两轮验证完成收口。
- Web/注入和认证授权是成熟度最高的两条主线。
- API、业务逻辑和密码学已有首个专用样板；云原生、Windows 主机和检测响应仍是主要结构性薄弱点。
