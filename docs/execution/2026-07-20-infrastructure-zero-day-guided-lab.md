...
在本机受控环境中完成 `infrastructure.zero-day` 学习实验，使用固定 `scenarioKey=synthetic-unknown-exploit-signal` 对比漏洞版与修复版，帮助学习者理解关键资产缺少分层防御、行为检测和快速隔离能力以及对应防御策略。

本实验模式为 `case-study`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/infrastructure/zero-day/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `infrastructure.zero-day` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/infrastructure/zero-day/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `compensating-control-missing` 观察 `infrastructure-zero-day-risk-accepted`。
6. 修复版使用 `compensating-control-missing` 观察 `infrastructure-zero-day-defense-blocked`。
7. 修复版使用 `exposure-contained-and-monitored` 验证正常受控流程仍可继续。
...
- 固定案例：虚构未知漏洞告警。
- 漏洞根因：关键资产缺少分层防御、行为检测和快速隔离能力。
- 核心防御：验证落实“攻击面收敛、虚拟补丁、行为检测、隔离、威胁狩猎和应急响应”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
...
- `pnpm --filter @network-safe/web exec tsx ../../tools/lab-scripts/infrastructure/zero-day/verify.ts`
- `pnpm --filter @network-safe/shared test`
- 通用服务与 API 测试逐项覆盖 `infrastructure.zero-day`。
- 前端 API、路由和工作台组件测试。
- `git diff --check`。
