...
在本机受控环境中完成 `network.eavesdropping` 学习实验，使用固定 `scenarioKey=synthetic-cleartext-session` 对比漏洞版与修复版，帮助学习者理解敏感通信使用明文协议或未验证的加密通道以及对应防御策略。

本实验模式为 `simulation`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/network/eavesdropping/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `network.eavesdropping` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/network/eavesdropping/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `transport-encryption-missing` 观察 `network-eavesdropping-risk-accepted`。
6. 修复版使用 `transport-encryption-missing` 观察 `network-eavesdropping-defense-blocked`。
7. 修复版使用 `encrypted-channel-verified` 验证正常受控流程仍可继续。
...
- 固定案例：合成明文会话摘要。
- 漏洞根因：敏感通信使用明文协议或未验证的加密通道。
- 核心防御：验证落实“端到端加密、证书验证、网络分段和敏感元数据最小化”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
...
- `pnpm --filter @network-safe/web exec tsx ../../tools/lab-scripts/network/eavesdropping/verify.ts`
- `pnpm --filter @network-safe/shared test`
- 通用服务与 API 测试逐项覆盖 `network.eavesdropping`。
- 前端 API、路由和工作台组件测试。
- `git diff --check`。
