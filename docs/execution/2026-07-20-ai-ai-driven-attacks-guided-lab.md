...
在本机受控环境中完成 `ai.ai-driven-attacks` 学习实验，使用固定 `scenarioKey=synthetic-automated-attack-chain` 对比漏洞版与修复版，帮助学习者理解自动化系统拥有过宽工具权限且缺少人工确认和审计以及对应防御策略。

本实验模式为 `case-study`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/ai/ai-driven-attacks/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `ai.ai-driven-attacks` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/ai/ai-driven-attacks/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `agent-guardrail-missing` 观察 `ai-ai-driven-attacks-risk-accepted`。
6. 修复版使用 `agent-guardrail-missing` 观察 `ai-ai-driven-attacks-defense-blocked`。
7. 修复版使用 `human-tool-policy-verified` 验证正常受控流程仍可继续。
...
- 固定案例：虚构自动化攻击链。
- 漏洞根因：自动化系统拥有过宽工具权限且缺少人工确认和审计。
- 核心防御：验证落实“工具允许列表、人工审批、速率限制、输出策略和完整审计”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
...
- 通用服务与 API 测试逐项覆盖 `ai.ai-driven-attacks`。
- 前端 API、路由和工作台组件测试。
- `git diff --check`。
