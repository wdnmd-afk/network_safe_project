...
在本机受控环境中完成 `auth.credential-stuffing` 学习实验，使用固定 `scenarioKey=reused-credential-batch` 对比漏洞版与修复版，帮助学习者理解认证只判断单次口令结果且没有设备、速率和泄露凭据风险关联以及对应防御策略。

本实验模式为 `interactive`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/auth/credential-stuffing/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `auth.credential-stuffing` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/auth/credential-stuffing/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `risk-correlation-missing` 观察 `auth-credential-stuffing-risk-accepted`。
6. 修复版使用 `risk-correlation-missing` 观察 `auth-credential-stuffing-defense-blocked`。
7. 修复版使用 `adaptive-challenge-enabled` 验证正常受控流程仍可继续。
...
- 固定案例：重复凭据登录批次。
- 漏洞根因：认证只判断单次口令结果且没有设备、速率和泄露凭据风险关联。
- 核心防御：验证落实“速率限制、泄露凭据检测、自适应验证和异常登录告警”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
...
| 固定案例被扩展为真实能力 | 超出项目边界 | 只处理固定虚构身份与会话数据，不读取真实账号、密码、Cookie 或 token。 |
...
- 通用服务与 API 测试逐项覆盖 `auth.credential-stuffing`。
- 前端 API、路由和工作台组件测试。
- `git diff --check`。
