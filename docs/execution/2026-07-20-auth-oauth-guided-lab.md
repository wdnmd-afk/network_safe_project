...
在本机受控环境中完成 `auth.oauth` 学习实验，使用固定 `scenarioKey=tampered-authorization-response` 对比漏洞版与修复版，帮助学习者理解授权响应没有与原始客户端、回调地址和请求上下文严格绑定以及对应防御策略。

本实验模式为 `interactive`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/auth/oauth/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `auth.oauth` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/auth/oauth/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `authorization-binding-missing` 观察 `auth-oauth-risk-accepted`。
6. 修复版使用 `authorization-binding-missing` 观察 `auth-oauth-defense-blocked`。
7. 修复版使用 `pkce-state-verified` 验证正常受控流程仍可继续。
...
- 固定案例：授权响应关联缺失。
- 漏洞根因：授权响应没有与原始客户端、回调地址和请求上下文严格绑定。
- 核心防御：验证落实“精确回调地址、state、nonce、PKCE 和最小 scope”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
