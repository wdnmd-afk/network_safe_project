...
在本机受控环境中完成 `supply-chain.malicious-package` 学习实验，使用固定 `scenarioKey=synthetic-package-change` 对比漏洞版与修复版，帮助学习者理解构建流程自动信任依赖更新且没有来源和行为审查以及对应防御策略。

本实验模式为 `case-study`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/supply-chain/malicious-package/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `supply-chain.malicious-package` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/supply-chain/malicious-package/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `package-review-missing` 观察 `supply-chain-malicious-package-risk-accepted`。
6. 修复版使用 `package-review-missing` 观察 `supply-chain-malicious-package-defense-blocked`。
7. 修复版使用 `provenance-and-policy-verified` 验证正常受控流程仍可继续。
...
- 固定案例：虚构依赖包异常变更。
- 漏洞根因：构建流程自动信任依赖更新且没有来源和行为审查。
- 核心防御：验证落实“来源证明、锁文件、脚本禁用策略、依赖审查和隔离构建”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
...
| 固定案例被扩展为真实能力 | 超出项目边界 | 只分析固定清单和来源记录，不安装、发布或下载任何依赖。 |
...
- `pnpm --filter @network-safe/web exec tsx ../../tools/lab-scripts/supply-chain/malicious-package/verify.ts`
- `pnpm --filter @network-safe/shared test`
- 通用服务与 API 测试逐项覆盖 `supply-chain.malicious-package`。
- 前端 API、路由和工作台组件测试。
- `git diff --check`。
