...
# Formjacking受控学习实验执行文档

## 1. 目标

在本机受控环境中完成 `client.formjacking` 学习实验，使用固定 `scenarioKey=synthetic-checkout-target-change` 对比漏洞版与修复版，帮助学习者理解敏感页面信任未受约束的第三方脚本和提交目标以及对应防御策略。

本实验模式为 `simulation`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/client/formjacking/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `client.formjacking` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/client/formjacking/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `form-integrity-missing` 观察 `client-formjacking-risk-accepted`。
6. 修复版使用 `form-integrity-missing` 观察 `client-formjacking-defense-blocked`。
7. 修复版使用 `csp-sri-target-verified` 验证正常受控流程仍可继续。
...
- 固定案例：虚构结账提交目标变更。
- 漏洞根因：敏感页面信任未受约束的第三方脚本和提交目标。
- 核心防御：验证落实“CSP、SRI、脚本清单、提交目标校验和敏感字段隔离”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
...
- 通用服务与 API 测试逐项覆盖 `client.formjacking`。
- 前端 API、路由和工作台组件测试。
- `git diff --check`。
