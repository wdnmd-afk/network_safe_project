...
在本机受控环境中完成 `infrastructure.container-escape` 学习实验，使用固定 `scenarioKey=synthetic-container-boundary-gap` 对比漏洞版与修复版，帮助学习者理解容器获得接近宿主的权限、挂载和内核能力以及对应防御策略。

本实验模式为 `case-study`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/infrastructure/container-escape/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `infrastructure.container-escape` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/infrastructure/container-escape/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `runtime-policy-missing` 观察 `infrastructure-container-escape-risk-accepted`。
6. 修复版使用 `runtime-policy-missing` 观察 `infrastructure-container-escape-defense-blocked`。
7. 修复版使用 `sandbox-policy-verified` 验证正常受控流程仍可继续。
...
- 固定案例：虚构容器隔离缺口。
- 漏洞根因：容器获得接近宿主的权限、挂载和内核能力。
- 核心防御：验证落实“非特权运行、能力最小化、只读文件系统、沙箱和运行时检测”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
...
| 固定案例被扩展为真实能力 | 超出项目边界 | 只审阅固定虚拟配置和事件，不连接或修改真实基础设施。 |
...
- `pnpm --filter @network-safe/web exec tsx ../../tools/lab-scripts/infrastructure/container-escape/verify.ts`
- `pnpm --filter @network-safe/shared test`
- 通用服务与 API 测试逐项覆盖 `infrastructure.container-escape`。
- 前端 API、路由和工作台组件测试。
- `git diff --check`。
