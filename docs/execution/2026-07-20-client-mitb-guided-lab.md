...
在本机受控环境中完成 `client.mitb` 学习实验，使用固定 `scenarioKey=synthetic-transaction-view-mismatch` 对比漏洞版与修复版，帮助学习者理解高风险交易确认完全依赖可能被篡改的浏览器上下文以及对应防御策略。

本实验模式为 `case-study`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/client/mitb/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `client.mitb` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/client/mitb/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `browser-view-trusted` 观察 `client-mitb-risk-accepted`。
6. 修复版使用 `browser-view-trusted` 观察 `client-mitb-defense-blocked`。
7. 修复版使用 `transaction-signing-verified` 验证正常受控流程仍可继续。
...
- 固定案例：虚构交易视图不一致。
- 漏洞根因：高风险交易确认完全依赖可能被篡改的浏览器上下文。
- 核心防御：验证落实“交易签名、带外确认、受信终端和服务端风险校验”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
...
- `pnpm --filter @network-safe/web exec tsx ../../tools/lab-scripts/client/mitb/verify.ts`
- `pnpm --filter @network-safe/shared test`
- 通用服务与 API 测试逐项覆盖 `client.mitb`。
- 前端 API、路由和工作台组件测试。
- `git diff --check`。
