...
在本机受控环境中完成 `client.drive-by-download` 学习实验，使用固定 `scenarioKey=synthetic-silent-download-chain` 对比漏洞版与修复版，帮助学习者理解浏览器和终端允许不可信内容触发下载或执行链以及对应防御策略。

本实验模式为 `case-study`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/client/drive-by-download/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `client.drive-by-download` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/client/drive-by-download/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `download-policy-missing` 观察 `client-drive-by-download-risk-accepted`。
6. 修复版使用 `download-policy-missing` 观察 `client-drive-by-download-defense-blocked`。
7. 修复版使用 `browser-isolation-verified` 验证正常受控流程仍可继续。
...
- 固定案例：虚构隐式下载事件链。
- 漏洞根因：浏览器和终端允许不可信内容触发下载或执行链。
- 核心防御：验证落实“浏览器更新、下载确认、内容隔离、应用控制和终端检测”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
...
- `pnpm --filter @network-safe/web exec tsx ../../tools/lab-scripts/client/drive-by-download/verify.ts`
- `pnpm --filter @network-safe/shared test`
- 通用服务与 API 测试逐项覆盖 `client.drive-by-download`。
- 前端 API、路由和工作台组件测试。
- `git diff --check`。
