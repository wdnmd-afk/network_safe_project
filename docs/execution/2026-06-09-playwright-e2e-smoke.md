...
1. 写 Playwright 配置测试，确认：
   - baseURL 使用 `http://127.0.0.1:6666`
   - Chromium 启动参数允许访问 `6666`
   - E2E 只跑 Chromium 项目
...
前端端口固定为 `6666`。该端口属于部分浏览器的不安全端口列表，浏览器可能默认阻止访问。

因此 Playwright Chromium 项目必须显式配置：

```text
--explicitly-allowed-ports=6666
```

HTTP 级冒烟测试已经使用 Node `http` / `https` 避开该限制；浏览器级测试必须在浏览器启动参数中处理。

## 5. 潜在风险分析

- 若本机未安装 Playwright 浏览器，首次运行可能需要安装浏览器依赖。
- 若 `6666` 或 `6667` 被非本项目服务占用，测试应失败，而不是误判通过。
