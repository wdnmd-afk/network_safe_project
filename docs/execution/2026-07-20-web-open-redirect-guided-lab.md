...
在本机受控环境中完成 `web.open-redirect` 学习实验，使用固定 `scenarioKey=untrusted-return-target` 对比漏洞版与修复版，帮助学习者理解服务端直接信任跳转目标且没有规范化与允许列表以及对应防御策略。

本实验模式为 `interactive`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/web/open-redirect/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `web.open-redirect` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/web/open-redirect/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `target-allowlist-missing` 观察 `web-open-redirect-risk-accepted`。
6. 修复版使用 `target-allowlist-missing` 观察 `web-open-redirect-defense-blocked`。
7. 修复版使用 `relative-path-verified` 验证正常受控流程仍可继续。
...
- 固定案例：未受信任返回地址。
- 漏洞根因：服务端直接信任跳转目标且没有规范化与允许列表。
- 核心防御：验证落实“站内相对路径约束、规范化和目标允许列表”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
...
- 通用服务与 API 测试逐项覆盖 `web.open-redirect`。
- 前端 API、路由和工作台组件测试。
- `git diff --check`。
