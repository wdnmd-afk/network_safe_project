...
在本机受控环境中完成 `client.malicious-extension` 学习实验，使用固定 `scenarioKey=synthetic-extension-permission-change` 对比漏洞版与修复版，帮助学习者理解浏览器允许扩展自动获得过宽权限且缺少来源审查以及对应防御策略。

本实验模式为 `case-study`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/client/malicious-extension/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `client.malicious-extension` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/client/malicious-extension/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `extension-review-missing` 观察 `client-malicious-extension-risk-accepted`。
6. 修复版使用 `extension-review-missing` 观察 `client-malicious-extension-defense-blocked`。
7. 修复版使用 `managed-extension-verified` 验证正常受控流程仍可继续。
...
- 固定案例：虚构扩展权限升级。
- 漏洞根因：浏览器允许扩展自动获得过宽权限且缺少来源审查。
- 核心防御：验证落实“企业允许列表、最小权限、更新审计和敏感站点隔离”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
...
| 固定案例被扩展为真实能力 | 超出项目边界 | 只使用固定浏览器行为案例，不注入真实页面、不采集表单数据。 |
...
- `pnpm --filter @network-safe/web exec tsx ../../tools/lab-scripts/client/malicious-extension/verify.ts`
- `pnpm --filter @network-safe/shared test`
- 通用服务与 API 测试逐项覆盖 `client.malicious-extension`。
- 前端 API、路由和工作台组件测试。
- `git diff --check`。
