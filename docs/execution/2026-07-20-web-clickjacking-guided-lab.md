...
在本机受控环境中完成 `web.clickjacking` 学习实验，使用固定 `scenarioKey=embedded-approval-overlay` 对比漏洞版与修复版，帮助学习者理解页面允许被任意来源嵌入且敏感动作缺少二次确认以及对应防御策略。

本实验模式为 `interactive`，状态达到 `ready` 只代表本项目内固定学习闭环可运行、可观察、可验证，不代表具备对外攻击能力。

## 2. 范围

- 建立标准目录 `labs/web/clickjacking/`。
- 建立漏洞版 / 修复版通用工作台入口。
...
1. 从共享目录读取 `web.clickjacking` 的准确字段和固定案例。
2. 生成实验元数据、README、漏洞版 / 修复版 / mock 说明和攻防文档。
3. 将 `/labs/web/clickjacking/vuln` 与 `/fixed` 接入通用工作台。
4. 将工作台配置和评估请求接入通用受控服务。
5. 漏洞版使用 `frame-policy-missing` 观察 `web-clickjacking-risk-accepted`。
6. 修复版使用 `frame-policy-missing` 观察 `web-clickjacking-defense-blocked`。
7. 修复版使用 `frame-policy-enforced` 验证正常受控流程仍可继续。
...
- 固定案例：透明覆盖审批按钮。
- 漏洞根因：页面允许被任意来源嵌入且敏感动作缺少二次确认。
- 核心防御：验证落实“CSP frame-ancestors、X-Frame-Options 和敏感动作确认”后的正常受控流程。
- 页面只提供固定选项，不提供任意输入框。
...
| 固定案例被扩展为真实能力 | 超出项目边界 | 只处理固定本机 Web 教学场景，不生成可用于外部站点的攻击载荷。 |
...
- 通用服务与 API 测试逐项覆盖 `web.clickjacking`。
- 前端 API、路由和工作台组件测试。
- `git diff --check`。
