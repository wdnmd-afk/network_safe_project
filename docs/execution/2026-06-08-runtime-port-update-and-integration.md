...
2. 新增前端 Vite 配置测试，确认开发端口为 `6670`，`/api` 代理指向 `http://localhost:6667`。
3. 修改后端启动入口使用统一配置函数。
...
6. 运行最小验证：
   ...
   - 启动后端 `6667`
   - 启动前端 `6670`
   - 请求 `http://127.0.0.1:6670/api/health`
...
- 本机 `6670` 或 `6667` 可能已被其他进程占用，联调前需要检查端口。
...
```powershell
Invoke-WebRequest -Uri http://127.0.0.1:6670/api/health -UseBasicParsing
```
