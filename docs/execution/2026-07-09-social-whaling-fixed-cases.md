...
本轮目标是在 `social/whaling` 已建立标准目录、基础文档和 `planned` 元数据的基础上，补齐固定虚构案例文档，让后续后端固定案例 API 和前端固定案例工作台有明确的案例边界。

本轮只补充文档、元数据 docs 入口和对应元数据测试断言，不实现页面、API、数据库写入、事件日志写入、`verify.ts`、`exploit.py`、真实高管画像采集、真实组织结构收集、真实投递、凭据收集、模板生成、第三方平台调用、会议邀请、付款指令或攻击脚本。
...
3. 明确禁止完整邮件正文、IM 对话、会议邀请模板、可复制标题、可投递附件名、真实链接、付款指令、跟踪链接、模板库、群发脚本和第三方平台调用。
...
- 固定案例 key 使用边界设计已规划的四类：`executive-wire-approval`、`board-confidential-request`、`legal-settlement-transfer`、`ma-data-room-access`。
- 虚构角色标签只使用泛化标签，例如“请求方标签”“执行动作标签”“流程标签”“核验标签”，不写真实姓名、公司、邮箱、手机号、部门、职位、董事会关系或外部顾问信息。
- 案例内容必须保留攻击方视角，但只解释误判机制，不提供可投递素材。
- 防御动作必须覆盖大额付款双人复核、可信通道回拨、法务核验、董事会固定通道、例外冻结、最小授权、隔离举报和日志复盘。
...
| 风险 | 影响 | 控制方式 |
|---|---|---|
| 固定案例写成完整话术 | 可能被误用为可投递模板、会议邀请或付款指令 | 只写线索卡、风险标签和流程节点，不写正文、标题、对话或付款细节 |
| 案例角色过于真实 | 可能引入隐私、骚扰或诈骗素材风险 | 只使用虚构角色标签和业务链路标签 |
...
- 后续后端 API 可以复用这些固定案例 key，但只能读取固定 key 和固定核验策略 key。
- 后续前端页面可以把 fixed-cases 作为案例卡内容来源的文档依据，但仍不得提供任意正文、邮箱、链接、附件、人员信息、付款信息或组织结构输入。
...
- 安全关键词扫描，确认真实投递、画像采集、凭据收集、模板生成、第三方平台调用、会议邀请、付款指令和攻击脚本相关命中均为禁止性说明、安全边界说明或测试反向断言。
...
- 新增 `labs/social/whaling/docs/fixed-cases.md`，记录四个固定虚构高层决策案例卡。
- 更新 `labs/social/whaling/meta.json`，新增 `fixed-cases` docs 入口。
- 同步捕鲸攻击 README、mock 说明、攻击方观察步骤和手动验证文档，明确固定案例只作为线索卡、风险标签、流程节点和复盘问题使用。
- 更新共享元数据测试，确认 docs path 包含 `labs/social/whaling/docs/fixed-cases.md`。
