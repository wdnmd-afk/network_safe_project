...
import {
  collectExpressApiRoutes,
  verifyApiEntrypointConsistency,
  type ApiEntrypointConsistencyError,
} from "../../apps/server/src/api-entrypoint-consistency.js";
...
export function verifyRepositoryApiEntrypoints() {
  const loaded = loadMetadata();
  const routes = collectExpressApiRoutes(createApp());
  const report = verifyApiEntrypointConsistency(loaded.metadata, routes);
