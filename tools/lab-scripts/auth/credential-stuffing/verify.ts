...
export type CredentialStuffingConsistencyCheck = {
  key: string;
  passed: boolean;
  message: string;
};

export type CredentialStuffingConsistencyReport = {
  labKey: "auth.credential-stuffing";
  scope: "local-repository-only";
  ok: boolean;
  checkedFiles: string[];
  checks: CredentialStuffingConsistencyCheck[];
  notes: string[];
};
...
const expectedApiEntrypoints = [
  ...
  "/api/labs/auth/credential-stuffing/fixed/evaluate",
];
...
const expectedDocs = [
  "labs/auth/credential-stuffing/README.md",
  "labs/auth/credential-stuffing/vuln/README.md",
  "labs/auth/credential-stuffing/fixed/README.md",
  "labs/auth/credential-stuffing/mock/README.md",
  "labs/auth/credential-stuffing/docs/attack-steps.md",
  "labs/auth/credential-stuffing/docs/fix-notes.md",
  "labs/auth/credential-stuffing/docs/manual-verification.md",
  "tools/lab-scripts/auth/credential-stuffing/README.md",
];

const implementationFiles = [
  "apps/server/src/services/credential-stuffing-lab.ts",
  "apps/server/src/app.ts",
  "apps/web/src/api/credential-stuffing-lab.ts",
  "apps/web/src/labs/credential-stuffing.ts",
  "apps/web/src/views/CredentialStuffingLabView.vue",
];
...
function createCheck(
  ...
  passed: boolean,
  message: string,
): CredentialStuffingConsistencyCheck {
  return {
    key,
    passed,
    message,
...
export function runCredentialStuffingConsistencyVerification(): CredentialStuffingConsistencyReport {
  const checks: CredentialStuffingConsistencyCheck[] = [];
  const metadataText = loadRepositoryText(
    "labs/auth/credential-stuffing/meta.json",
  );
  ...
  if (!parsedMetadata.ok) {
    return {
      ...
      scope: "local-repository-only",
      ok: false,
      checkedFiles: ["labs/auth/credential-stuffing/meta.json"],
      checks,
  ...
  const scriptPath = "tools/lab-scripts/auth/credential-stuffing/verify.ts";
  ...
  checks.push(
    createCheck(
      ...
      metadata.id === "auth.credential-stuffing" &&
        metadata.mode === "interactive" &&
        metadata.status === "ready",
      "凭据填充元数据应保持 auth.credential-stuffing / interactive / ready。",
    ),
  );
  checks.push(
    createCheck(
  ...
  checks.push(
    createCheck(
      ...
      "凭据填充 API 入口应只包含专用工作台配置与漏洞版/修复版评估接口。",
    ),
  );
  checks.push(
    createCheck(
      ...
      "凭据填充三个 canonical 终止信号应保持不变。",
    ),
  );
  checks.push(
    createCheck(
      "script-entrypoint",
      scriptEntrypoint?.key === "auth-credential-stuffing-verify" &&
        scriptEntrypoint.path === scriptPath &&
        scriptEntrypoint.language === "ts" &&
        scriptExists,
      "凭据填充脚本入口应登记本机只读一致性验证脚本。",
    ),
  );
  checks.push(
    createCheck(
      ...
      metadata.verification.automation.supported === true &&
        ...
        metadata.verification.automation.apiTest.specPath ===
          "apps/server/tests/credential-stuffing-lab.test.ts" &&
        ...
        hasExactValues(
          ...
          ["auth-credential-stuffing-verify"],
        ),
  ...
  checks.push(
    createCheck(
      ...
      "凭据填充文档应持续声明固定案例、固定 scenarioKey 和未知输入脱敏阻断边界。",
    ),
  );
  checks.push(
    createCheck(
  ...
  return {
    ...
    checks,
    notes: [
      "本脚本只读取仓库内凭据填充元数据、文档和实现文件。",
      "本脚本不发起 HTTP 请求，不访问外部目标，不读取真实账号或口令。",
      "ready 状态仅代表本项目内固定多步骤学习闭环完成，不代表提供真实攻击能力。",
    ],
  };
}

export function getCredentialStuffingConsistencyVerificationPlan() {
  return {
    labKey: "auth.credential-stuffing",
    scope: "local-repository-only",
    safeBoundary:
      "只验证本项目仓库内凭据填充文档、元数据、入口和安全边界一致性，不发起网络请求，不读取真实凭据。",
    expectedWebEntrypoints,
    expectedApiEntrypoints,
    expectedSignals,
    expectedScript: "tools/lab-scripts/auth/credential-stuffing/verify.ts",
...
if (
  ...
  const report = runCredentialStuffingConsistencyVerification();
