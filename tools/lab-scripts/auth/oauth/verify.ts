...
export type OauthConsistencyCheck = {
  key: string;
  passed: boolean;
  message: string;
};

export type OauthConsistencyReport = {
  labKey: "auth.oauth";
  scope: "local-repository-only";
  ok: boolean;
  checkedFiles: string[];
  checks: OauthConsistencyCheck[];
  notes: string[];
};
...
const expectedApiEntrypoints = [
  ...
  "/api/labs/auth/oauth/fixed/evaluate",
];

const expectedSignals = [
  "auth-oauth-risk-accepted",
  "auth-oauth-defense-blocked",
  "auth-oauth-normal-verified",
];

const expectedDocs = [
  "labs/auth/oauth/README.md",
  "labs/auth/oauth/vuln/README.md",
  "labs/auth/oauth/fixed/README.md",
  "labs/auth/oauth/mock/README.md",
  "labs/auth/oauth/docs/attack-steps.md",
  "labs/auth/oauth/docs/fix-notes.md",
  "labs/auth/oauth/docs/manual-verification.md",
  "tools/lab-scripts/auth/oauth/README.md",
];

const implementationFiles = [
  "apps/server/src/services/oauth-lab.ts",
  "apps/server/src/app.ts",
  "apps/web/src/api/oauth-lab.ts",
  "apps/web/src/labs/oauth.ts",
  "apps/web/src/views/OauthLabView.vue",
];
...
export function runOauthConsistencyVerification(): OauthConsistencyReport {
  const checks: OauthConsistencyCheck[] = [];
  const metadataText = loadRepositoryText("labs/auth/oauth/meta.json");
  ...
  if (!parsedMetadata.ok) {
    return {
      ...
      scope: "local-repository-only",
      ok: false,
      checkedFiles: ["labs/auth/oauth/meta.json"],
      checks,
  ...
  const scriptPath = "tools/lab-scripts/auth/oauth/verify.ts";
  ...
  checks.push(
    createCheck(
      ...
      metadata.id === "auth.oauth" &&
        metadata.mode === "interactive" &&
        metadata.status === "ready",
      "OAuth 漏洞元数据应保持 auth.oauth / interactive / ready。",
    ),
  );
  checks.push(
    createCheck(
  ...
  checks.push(
    createCheck(
      ...
      "OAuth 漏洞 API 入口应只包含专用工作台配置与漏洞版/修复版评估接口。",
    ),
  );
  checks.push(
    createCheck(
      ...
      "OAuth 漏洞三个 canonical 终止信号应保持不变。",
    ),
  );
  checks.push(
    createCheck(
      "script-entrypoint",
      scriptEntrypoint?.key === "auth-oauth-verify" &&
        scriptEntrypoint.path === scriptPath &&
        scriptEntrypoint.language === "ts" &&
        scriptExists,
      "OAuth 漏洞脚本入口应登记本机只读一致性验证脚本。",
    ),
  );
  checks.push(
    createCheck(
      ...
      metadata.verification.automation.supported === true &&
        ...
        metadata.verification.automation.apiTest.specPath ===
          "apps/server/tests/oauth-lab.test.ts" &&
  ...
  checks.push(
    createCheck(
      ...
      "OAuth 漏洞文档应持续声明固定案例、固定 scenarioKey 和未知输入脱敏阻断边界。",
    ),
  );
  checks.push(
    createCheck(
  ...
  return {
    ...
    checks,
    notes: [
      "本脚本只读取仓库内 OAuth 漏洞元数据、文档和实现文件。",
      "本脚本不发起 HTTP 请求，不访问外部目标，不执行真实授权流程。",
      "ready 状态仅代表本项目内固定多步骤学习闭环完成，不代表提供真实攻击能力。",
    ],
  };
}

export function getOauthConsistencyVerificationPlan() {
  return {
    labKey: "auth.oauth",
    scope: "local-repository-only",
    safeBoundary:
      "只验证本项目仓库内 OAuth 漏洞文档、元数据、入口和安全边界一致性，不发起网络请求，不执行真实授权流程。",
    expectedWebEntrypoints,
    expectedApiEntrypoints,
    expectedSignals,
    expectedScript: "tools/lab-scripts/auth/oauth/verify.ts",
...
if (
  ...
  const report = runOauthConsistencyVerification();
