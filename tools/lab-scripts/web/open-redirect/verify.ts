...
export type OpenRedirectConsistencyCheck = {
  key: string;
  passed: boolean;
  message: string;
};

export type OpenRedirectConsistencyReport = {
  labKey: "web.open-redirect";
  scope: "local-repository-only";
  ok: boolean;
  checkedFiles: string[];
  checks: OpenRedirectConsistencyCheck[];
  notes: string[];
};
...
const expectedApiEntrypoints = [
  ...
  "/api/labs/web/open-redirect/fixed/evaluate",
];

const expectedSignals = [
  "web-open-redirect-risk-accepted",
  "web-open-redirect-defense-blocked",
  "web-open-redirect-normal-verified",
];

const expectedDocs = [
  "labs/web/open-redirect/README.md",
  "labs/web/open-redirect/vuln/README.md",
  "labs/web/open-redirect/fixed/README.md",
  "labs/web/open-redirect/mock/README.md",
  "labs/web/open-redirect/docs/attack-steps.md",
  "labs/web/open-redirect/docs/fix-notes.md",
  "labs/web/open-redirect/docs/manual-verification.md",
  "tools/lab-scripts/web/open-redirect/README.md",
];

const implementationFiles = [
  "apps/server/src/services/open-redirect-lab.ts",
  "apps/server/src/app.ts",
  "apps/web/src/api/open-redirect-lab.ts",
  "apps/web/src/labs/open-redirect.ts",
  "apps/web/src/views/OpenRedirectLabView.vue",
];
...
export function runOpenRedirectConsistencyVerification(): OpenRedirectConsistencyReport {
  const checks: OpenRedirectConsistencyCheck[] = [];
  const metadataText = loadRepositoryText("labs/web/open-redirect/meta.json");
  ...
  if (!parsedMetadata.ok) {
    return {
      ...
      scope: "local-repository-only",
      ok: false,
      checkedFiles: ["labs/web/open-redirect/meta.json"],
      checks,
  ...
  const scriptPath = "tools/lab-scripts/web/open-redirect/verify.ts";
  ...
  checks.push(
    createCheck(
      ...
      metadata.id === "web.open-redirect" &&
        metadata.mode === "interactive" &&
        metadata.status === "ready",
      "开放重定向元数据应保持 web.open-redirect / interactive / ready。",
    ),
  );
  checks.push(
    createCheck(
  ...
  checks.push(
    createCheck(
      ...
      "开放重定向 API 入口应只包含专用工作台配置与漏洞版/修复版评估接口。",
    ),
  );
  checks.push(
    createCheck(
      ...
      "开放重定向三个 canonical 终止信号应保持不变。",
    ),
  );
  checks.push(
    createCheck(
      "script-entrypoint",
      scriptEntrypoint?.key === "web-open-redirect-verify" &&
        scriptEntrypoint.path === scriptPath &&
        scriptEntrypoint.language === "ts" &&
        scriptExists,
      "开放重定向脚本入口应登记本机只读一致性验证脚本。",
    ),
  );
  checks.push(
    createCheck(
      ...
      metadata.verification.automation.supported === true &&
        ...
        metadata.verification.automation.apiTest.specPath ===
          "apps/server/tests/open-redirect-lab.test.ts" &&
  ...
  checks.push(
    createCheck(
      ...
      "开放重定向文档应持续声明固定案例、固定 scenarioKey 和未知输入脱敏阻断边界。",
    ),
  );
  checks.push(
    createCheck(
  ...
  return {
    ...
    checkedFiles: [
      "labs/web/open-redirect/meta.json",
      ...expectedDocs,
      ...implementationFiles,
      scriptPath,
    ],
    checks,
    notes: [
      "本脚本只读取仓库内开放重定向元数据、文档和实现文件。",
      "本脚本不发起 HTTP 请求，不访问外部目标，不执行真实重定向。",
      "ready 状态仅代表本项目内固定多步骤学习闭环完成，不代表提供真实攻击能力。",
    ],
  };
}

export function getOpenRedirectConsistencyVerificationPlan() {
  return {
    labKey: "web.open-redirect",
    scope: "local-repository-only",
    safeBoundary:
      "只验证本项目仓库内开放重定向文档、元数据、入口和安全边界一致性，不发起网络请求，不执行真实重定向。",
    expectedWebEntrypoints,
    expectedApiEntrypoints,
    expectedSignals,
    expectedScript: "tools/lab-scripts/web/open-redirect/verify.ts",
...
if (
  ...
  const report = runOpenRedirectConsistencyVerification();
