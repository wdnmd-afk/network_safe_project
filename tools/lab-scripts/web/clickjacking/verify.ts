...
export type ClickjackingConsistencyCheck = {
  key: string;
  passed: boolean;
  message: string;
};

export type ClickjackingConsistencyReport = {
  labKey: "web.clickjacking";
  scope: "local-repository-only";
  ok: boolean;
  checkedFiles: string[];
  checks: ClickjackingConsistencyCheck[];
  notes: string[];
};
...
const expectedApiEntrypoints = [
  ...
  "/api/labs/web/clickjacking/fixed/evaluate",
];

const expectedSignals = [
  "web-clickjacking-risk-accepted",
  "web-clickjacking-defense-blocked",
  "web-clickjacking-normal-verified",
];

const expectedDocs = [
  "labs/web/clickjacking/README.md",
  "labs/web/clickjacking/vuln/README.md",
  "labs/web/clickjacking/fixed/README.md",
  "labs/web/clickjacking/mock/README.md",
  "labs/web/clickjacking/docs/attack-steps.md",
  "labs/web/clickjacking/docs/fix-notes.md",
  "labs/web/clickjacking/docs/manual-verification.md",
  "tools/lab-scripts/web/clickjacking/README.md",
];

const implementationFiles = [
  "apps/server/src/services/clickjacking-lab.ts",
  "apps/server/src/app.ts",
  "apps/web/src/api/clickjacking-lab.ts",
  "apps/web/src/labs/clickjacking.ts",
  "apps/web/src/views/ClickjackingLabView.vue",
];
...
export function runClickjackingConsistencyVerification(): ClickjackingConsistencyReport {
  const checks: ClickjackingConsistencyCheck[] = [];
  const metadataText = loadRepositoryText("labs/web/clickjacking/meta.json");
  ...
  if (!parsedMetadata.ok) {
    return {
      ...
      scope: "local-repository-only",
      ok: false,
      checkedFiles: ["labs/web/clickjacking/meta.json"],
      checks,
  ...
  const scriptPath = "tools/lab-scripts/web/clickjacking/verify.ts";
  ...
  checks.push(
    createCheck(
      ...
      metadata.id === "web.clickjacking" &&
        metadata.mode === "interactive" &&
        metadata.status === "ready",
      "点击劫持元数据应保持 web.clickjacking / interactive / ready。",
    ),
  );
  checks.push(
    createCheck(
  ...
  checks.push(
    createCheck(
      ...
      "点击劫持 API 入口应只包含专用工作台配置与漏洞版/修复版评估接口。",
    ),
  );
  checks.push(
    createCheck(
      ...
      "点击劫持三个 canonical 终止信号应保持不变。",
    ),
  );
  checks.push(
    createCheck(
      "script-entrypoint",
      scriptEntrypoint?.key === "web-clickjacking-verify" &&
        scriptEntrypoint.path === scriptPath &&
        scriptEntrypoint.language === "ts" &&
        scriptExists,
      "点击劫持脚本入口应登记本机只读一致性验证脚本。",
    ),
  );
  checks.push(
    createCheck(
      ...
      metadata.verification.automation.supported === true &&
        ...
        metadata.verification.automation.apiTest.specPath ===
          "apps/server/tests/clickjacking-lab.test.ts" &&
  ...
  checks.push(
    createCheck(
      ...
      "点击劫持文档应持续声明固定案例、固定 scenarioKey 和未知输入脱敏阻断边界。",
    ),
  );
  checks.push(
    createCheck(
  ...
  return {
    ...
    checkedFiles: [
      "labs/web/clickjacking/meta.json",
      ...expectedDocs,
      ...implementationFiles,
      scriptPath,
    ],
    checks,
    notes: [
      "本脚本只读取仓库内点击劫持元数据、文档和实现文件。",
      "本脚本不发起 HTTP 请求，不访问外部目标，不执行真实框架嵌入或点击劫持。",
      "ready 状态仅代表本项目内固定多步骤学习闭环完成，不代表提供真实攻击能力。",
    ],
  };
}

export function getClickjackingConsistencyVerificationPlan() {
  return {
    labKey: "web.clickjacking",
    scope: "local-repository-only",
    safeBoundary:
      "只验证本项目仓库内点击劫持文档、元数据、入口和安全边界一致性，不发起网络请求，不执行真实攻击。",
    expectedWebEntrypoints,
    expectedApiEntrypoints,
    expectedSignals,
    expectedScript: "tools/lab-scripts/web/clickjacking/verify.ts",
...
if (
  ...
  const report = runClickjackingConsistencyVerification();
