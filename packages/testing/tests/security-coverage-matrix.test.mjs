...
test("security coverage matrix matches all current lab metadata", () => {
  ...
  assert.equal(summary.matrixRows, 78);
  ...
  // LT-043 在 api 分类内新增 rate-limit-idempotency 专用 simulation；
  // LT-044 在 host 分类内新增 persistence-triage 专用 case-study。
  assert.equal(summary.dedicated, 48);
  ...
  assert.equal(summary.categories.host, 3);
